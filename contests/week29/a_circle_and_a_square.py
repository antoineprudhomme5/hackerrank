...
def is_in_circle(p_x, p_y, c_x, c_y, r):
    #if (p_x == 4 and p_y == 10) or (p_x == 5 and p_y == 10):
        #print("%d %d : %.6f => %.6f" % (p_x, p_y, math.sqrt(math.pow(p_x - c_x, 2) + math.pow(p_y - c_y, 2)), r))
        #print(math.sqrt(math.pow(p_x - c_x, 2) + math.pow(p_y - c_y, 2)) <= r)
...
def triangle_area(a_x, a_y, b_x, b_y, c_x, c_y):
    ...
    p = (a + b + c) / 2
    temp = p * (p-a) * (p-b) * (p-c)
    #if (b_x == 4 and b_y == 10) or (b_x == 5 and b_y == 10):
...
def is_in_square(p_x, p_y, a_x, a_y, b_x, b_y, c_x, c_y, d_x, d_y):
    ...
    #if p_x == 4 and p_y == 10:
        #print("%d %d : %d" % (p_x, p_y, int(areas_sum)))
