...
def is_in_cicle(p_x, p_y, c_x, c_y, r):
...
def triangle_area(a_x, a_y, b_x, b_y, c_x, c_y):
    ...
    p = (a + b + c) / 2
    temp = p * (p-a) * (p-b) * (p-c)
    return math.sqrt(temp) if temp > 0 else 50
...
for j in range(h):
    row = ['.'] * w
    for i in range(w):
        if is_in_cicle(i, j, circleX, circleY, r) or is_in_square(i, j, x1, y1, x2, y2, x3, y3, x4, y4):
            row[i] = '#'
    print(''.join(row))
